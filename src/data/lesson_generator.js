...
export function getLessonForModule(module) {
  ...
  if (slug === 'maths' || id === 1) return lesson_maths_trig;
  if (slug === 'physique' || id === 2) return lesson_physique;
  if (slug === 'chimie' || id === 3) return lesson_materiaux;
  if (slug === 'dessin' || id === 4) return lesson_dessin;
  if (slug === 'bim' || id === 5) return lesson_bim;
  if (slug === 'mecanique' || id === 6) return lesson_mecanique;
  if (slug === 'rdm' || id === 7) return lesson_rdm;
  if (slug === 'structures' || slug === 'analyse' || id === 8) return lesson_analyse;
  if (slug === 'beton-arme' || slug === 'beton_arme' || id === 9) return lesson_beton_arme;
  if (slug === 'beton-precontraint' || slug === 'precontrainte' || id === 10) return lesson_precontrainte;
  if (slug === 'metal' || id === 11) return lesson_metal;
  if (slug === 'bois' || id === 12) return lesson_bois;
  if (slug === 'geotechnique' || id === 13) return lesson_geotechnique;
  if (slug === 'hydraulique' || id === 14) return lesson_hydraulique;
  if (slug === 'routes' || id === 15) return lesson_routes;
  if (slug === 'ponts' || id === 16) return lesson_ponts;
  if (slug === 'tunnels' || id === 17) return lesson_tunnels;
  if (slug === 'barrages' || id === 18) return lesson_barrages;
  if (slug === 'aeroports' || id === 19) return lesson_aeroports;
  if (slug === 'ports' || id === 20) return lesson_ports;
  if (slug === 'ferroviaire' || id === 21) return lesson_ferroviaire;
  if (slug === 'topographie' || id === 22) return lesson_topographie;
  if (slug === 'materiaux' || id === 23) return lesson_materiaux;
  if (slug === 'chantier' || id === 24) return lesson_chantier;
  if (slug === 'projets' || slug === 'management' || id === 25) return lesson_management;
  if (slug === 'hse' || slug === 'fondations' || id === 26) return lesson_fondations;
  if (slug === 'environnement' || slug === 'seisme' || id === 27) return lesson_seisme;
  if (slug === 'normes' || slug === 'eco' || id === 28) return lesson_eco;
  if (slug === 'logiciels' || slug === 'metre' || id === 29) return lesson_metre;
  if (slug === 'ia' || slug === 'droit' || id === 30) return lesson_droit;
  if (slug === 'methodes' || slug === 'climat' || id === 31) return lesson_climat;
  if (slug === 'pathologie' || id === 32) return lesson_pathologie;
  if (slug === 'qualite' || slug === 'energie' || id === 33) return lesson_energie;
  if (slug === 'cas' || slug === 'bim_management' || id === 34) return lesson_bim_management;
  if (slug === 'carriere' || slug === 'ia_btp' || id === 35) return lesson_ia_btp;
