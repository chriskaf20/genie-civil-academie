...
export default function TrigSVG({ angle = 30, setAngle }) {
  ...
  const computeAngle = useCallback((event, svgEl) => {
    ...
    const scaleX = W / (bounds.width || W);
    const scaleY = H / (bounds.height || H);
    const vx = mx * scaleX;
    const vy = my * scaleY;
    const dx = vx - ox;
  ...
  const handlePointerDown = (e) => {
    ...
    if (a !== undefined && setAngle) setAngle(a);
  };
  ...
  return (
    <div className="w-full max-w-full overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-900/80 p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs uppercase tracking-widest text-sky-400 font-semibold">Diagramme Interactif</span>
        <span className="text-xs bg-slate-800 px-3 py-1 rounded-full text-slate-300 font-mono">θ = {safeAngle}°</span>
      </div>

      <div className="w-full max-w-full overflow-hidden rounded-xl bg-slate-950/60">
        <svg
          viewBox={`0 0 ${W} ${H}`}
          className="w-full h-auto max-w-full svg-interactive block"
          style={{ maxHeight: 220 }}
      ...
      <div className="mt-3 text-xs text-slate-500 text-center flex items-center justify-center gap-1.5 flex-wrap">
        <SafeInlineMath math="\sin^2\theta + \cos^2\theta = 1" />
        <span>— identité fondamentale</span>
      </div>
    </div>
  );
}
