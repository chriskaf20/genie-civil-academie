  ...
  return (
    <div className="flex flex-col h-full space-y-3">
      ...
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-2.5 flex items-center justify-between">
        ...
        <span className="tag-green text-[10px]">Illimité</span>
      </div>
      ...
      <div>
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-700 bg-slate-800/80 pl-9 pr-3 py-2 text-sm text-slate-200 placeholder:text-slate-600 focus:border-sky-500/50 transition-colors"
          />
        </div>
      </div>
          ...
          return (
            <div key={cat} className="rounded-xl overflow-hidden">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl border text-xs font-bold uppercase tracking-wider transition-colors ${color}`}
              >
                <span>{cat}</span>
                <div className="flex items-center gap-2">
                  <span className="opacity-70">{mods.length}</span>
                  <span className="transition-transform duration-200" style={{ transform: isCollapsed ? 'rotate(-90deg)' : 'rotate(0deg)' }}>▾</span>
                </div>
              </button>
                  ...
                  {mods.map(m => {
                    ...
                    return (
                      <button
                        key={m.slug}
                        onClick={() => onSelect(m)}
                        className={`module-nav-item w-full flex items-start gap-3 rounded-xl px-3 py-2.5 text-left transition-all ${
                          isActive
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            <span className="text-xs text-slate-500 font-mono shrink-0">{String(m.id).padStart(2, '0')}</span>
                            <div className="flex items-center gap-1">
                              {isDone && <span className="text-emerald-400 text-xs font-bold" title="Terminé">✓</span>}
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`text-[11px] ${LEVEL_COLORS[m.level] || 'text-slate-500'}`}>{m.level}</span>
                            <span className="text-slate-600 text-xs">·</span>
                            <span className="text-[11px] text-slate-400">{m.duration}</span>
                          </div>
                        </div>
                      </button>
                    );
